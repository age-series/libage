...
data class Spline3d(val segments: SplineSegmentMap<SplineSegment3d>) :
    ...
    override fun evaluateDual(t: Double, n: Int) = segments[t].let { it.evaluateDual(
        Dual.variable(
            it.segmentAbscissa(t),
            n
        )
    ) }
...
fun <T> T.adaptscan(t0: Double, t1: Double, tIncrMax: Double, iMax: Int = Int.MAX_VALUE, condition: AdaptscanCondition<T>, ): ArrayList<Double>? {
